...
goalXBin <- 80/input$goalXBin
goalYBin <- 120/input$goalYBin
...
heatmap$xbin <- cut(heatmap$location.x, breaks = seq(from=0, to=120, by = goalYBin),include.lowest=TRUE )
heatmap$ybin <- cut(heatmap$location.y, breaks = seq(from=0, to=80, by = goalXBin),include.lowest=TRUE) #2



#dataframe for shot assits
goalAssistZoneTeam <- heatmap%>%
  ...
  summarise(total_DA = max(total_DA),
            ...
            location.x = (ceiling(median(location.x)/goalYBin)*goalYBin)-(goalYBin/2),
            location.y = (ceiling(median(location.y)/goalXBin)*goalXBin)-(goalXBin/2)
  )%>%
...
goalAssistZoneTotal <-   goalAssistZoneTeam %>%
  group_by(xbin, ybin, location.x, location.y) %>%
  summarize(total_occurrences = sum(bin_DA))

ggplot(data= goalAssistZoneTotal, aes(x = location.y, y = location.x, fill = total_occurrences, group = total_occurrences)) +
  geom_bin2d(binwidth = c(goalXBin, goalYBin), position = "identity", alpha = 0.9) +
  ...
  geom_label(aes(label = total_occurrences),
             colour=inside.lines,
             label.size = NA,
             fill="#131313",
             family = fontFamily,
