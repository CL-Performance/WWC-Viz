...
shotAssistZoneTotal <-   shotAssistZoneTeam %>%
  group_by(xbin, ybin, location.x, location.y) %>%
  summarize(total_occurrences = sum(bin_DA))

ggplot(data= shotAssistZoneTotal, aes(x = location.y, y = location.x, fill = total_occurrences, group = total_occurrences)) +
