...
ggplot(data= goalAssistZoneTeam, aes(x = location.y, y = location.x, fill = diff_vs_ave, group = diff_vs_ave)) +
  ...
  scale_fill_gradientn(colours = zoneColours, trans = "reverse", labels =
                         scales::percent_format(accuracy = 1), limits = c(max(goalAssistZoneTeam$diff_vs_ave), min(goalAssistZoneTeam$diff_vs_ave))) +
